...
export const signUp = asyncHandler(async (req, res) => {
    ...
    // Send email
    const token = jsonwebtoken.sign({ email }, process.env.JWT_SECRET, { expiresIn: '1d' });
    ...
    await member.save();
    res.status(201).json({ message: 'Member registered successfully, waiting on email confirmation' });
  });
...
export const verifyAccount = asyncHandler(async (req, res, next) => {
    ...
    const member = await Member.findOneAndUpdate({email: decode.email,verifyEmail:false},{verifyEmail:true})
    if(!member){
         return next(new AppError("Member NotFound",404))
    }
    return res.status(200).json({message: "Member has verified the account",success: true})
})
...
export const resendVerificationEmail = asyncHandler(async (req, res, next) => {
  ...
  if (!member) {
    return res.status(404).json({ message: 'No unverified account found for this email.' });
  }
  ...
  if (member.lastEmailSentAt && (now - member.lastEmailSentAt.getTime()) < minTimeBetweenEmails) {
    return res.status(429).json({ message: 'Please wait before requesting another verification email.' });
  }

  // Generate a new verification token
  const token = jsonwebtoken.sign({ email: member.email }, process.env.JWT_SECRET, { expiresIn: '1d' });
...
export const login = asyncHandler(async (req, res) => {
    ...
    if (!member) {
        ...
        if (!member.verifyEmail) {
          return res.status(400).json({ message: 'Email not confirmed. Please verify your email before logging in.' }).redirect(`http://localhost:${port}/members/resend-verify-email`);
      }
...
export const updateEmail = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json({ 
      message: "Verification code sent to new email. Please verify to complete the update." 
  });
});
...
cron.schedule('0 0 * * *', asyncHandler(async () => { const cleanupExpiredTokens = asyncHandler(async () => {
  const now = new Date();
